<?php
	function db_connect(){
		$conn = mysqli_connect("localhost", "root", "", "online_books");
		if(!$conn){
	...
	function select4LatestBook($conn){
		$row = array();
		$query = "SELECT book_isbn, book_image FROM books ORDER BY book_isbn DESC limit 8";
	...
	function getAllUsers($conn){
		...
		$result =mysqli_query($conn,$query);
		if(!$result){
			echo "cant retrieve data " . mysqli_error($conn);
			exit;
		}
		return $result;
	}
